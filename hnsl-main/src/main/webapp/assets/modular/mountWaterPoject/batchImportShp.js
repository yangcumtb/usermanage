layui.use(['table', 'layarea', 'admin', 'form', 'HttpRequest', 'func', 'upload', 'element', 'layer'], function () {
    ...
    form.on('select(shptype)', function (data) {
        if (data.value === 'dtupload') {
            ...
            var uploadListIns = upload.render({
                elem: '#testList',
                elemList: $('#fileList'),
                url: Feng.ctxPath + '/mountwaterCommon/mountWaterImportShp',
                accept: 'file',
                exts: 'zip',
                multiple: true,
                data: {
                ...
                before: function (obj) {
                    if ($('#xmnf').val() == null || $('#zXmmc').val() == null || $('#gcXmmc').val() == null || $('#num').val() == null) {
                        return false
                    }
                    //obj参数包含的信息，跟 choose回调完全一致
                    this.url = Feng.ctxPath + '/mountwaterCommon/mountWaterImportShp';
    ...
    //     elem: '#testList',
    //     elemList: $('#fileList'),
    //     url: './proImportShp?shpType=' + shpType,
    //     accept: 'file',
    //     exts: 'zip',
    //     multiple: true,
    //     data: {
    ...
    //     before: function (obj) {
    //         //obj参数包含的信息，跟 choose回调完全一致
    //         this.url = './proImportShp?shpType=' + shpType;
    //         loadingIndex = layer.load(); //上传loading
    //         layer.open({
    //             type: 1,
    //             title: "上传进度", //不显示标题
