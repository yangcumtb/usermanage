layui.use(['admin', 'HttpRequest', 'upload', 'form', 'layer', 'element'], function () {
    ...
    //获取associateid
    var ajaxassociateid = new HttpRequest(Feng.ctxPath + '/prominemanger/plannedRecoverspot/detail?id=' + keyIdValue, 'get');
    var result = ajaxassociateid.start();
    associateid = keyIdValue;
    ...
    $.ajax({
        type: 'get',
        url: Feng.ctxPath + '/prominemanger/plannedRecoverspot/fileDetail/' + $("#keyIdValue").val(),
                ...
                if (res.data[i].fileBusinessName == "规划图" || res.data[i].fileBusinessName == "图斑影像图" || res.data[i].fileBusinessName == "核查") {
                    ...
                    tr.find('.file-delete').on('click', function (data) {
                        ...
                        var operation = function () {
                            ...
                            var ajax = new HttpRequest(Feng.ctxPath + '/prominemanger/plannedRecoverspot/deleteFileByFileId', 'post', function (data) {
                                Feng.success("删除成功!");
                                tr.remove();
                            }, function (data) {
                        ...
                        Feng.confirm(" 是否删除? 如果确认删除，后端将会删除图片记录、图片缩略图以及图片。", operation);
                    });
