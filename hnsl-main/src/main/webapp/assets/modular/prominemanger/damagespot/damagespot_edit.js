...
layui.use(['form', 'admin', 'HttpRequest', 'layarea', 'laydate', 'upload', 'formSelects', 'laydate', 'element', 'xmSelect'], function () {
    ...
    if (result.data.damagespotParam.hcrq != null) {
        result.data.damagespotParam.hcrq = result.data.damagespotParam.hcrq.substr(0, 7);
    }
    //多选回显
    if (result.data.damagespotParam.sddlV != null) {
        var sd = result.data.damagespotParam.sddlV.split(',');
        console.log(sd)
    ...
    var data1 = result.data.damagespotParam;
    var type = data1.shlx;
    var dzlx = data1.dztblx;
//获取数据库数据回显
    if (type == 'WS') {
        $("#ws1").css("display", "");
...
//监控shlx
    ...
    form.on('submit(btnSubmit)', function (data) {
        ...
        var ajax = new HttpRequest(Feng.ctxPath + "/prominemanger/damagespot/edit", 'post', function (data) {
            Feng.success("更新成功！");
            //传给上个页面，刷新table用
