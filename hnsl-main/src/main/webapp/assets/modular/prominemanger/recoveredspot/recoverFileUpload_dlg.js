layui.use(['admin', 'HttpRequest', 'upload', 'form', 'layer', 'element'], function () {
    ...
    //获取associateid
    var ajaxassociateid = new HttpRequest(Feng.ctxPath + '/prominemanger/recoveredspot/detail?id=' + keyIdValue, 'get');
    ...
    $.ajax({
        type: 'get',
        url: Feng.ctxPath + '/prominemanger/recoveredspot/fileDetail/' + $("#keyIdValue").val(),
                ...
                if (res.data[i].fileBusinessName == "规划图" || res.data[i].fileBusinessName == "图斑影像图" || res.data[i].fileBusinessName == "核查") {
                    ...
                    tr.find('.file-delete').on('click', function (data) {
                        ...
                        var operation = function () {
                            ...
                            // data[field] = fileID;
                            var ajax = new HttpRequest(Feng.ctxPath + '/prominemanger/recoveredspot/deleteFileByFileId', 'post', function (data) {
                                Feng.success("删除成功!");
                                tr.remove();
                            }, function (data) {
