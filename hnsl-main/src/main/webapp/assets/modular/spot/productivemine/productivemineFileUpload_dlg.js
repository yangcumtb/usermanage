layui.use(['admin', 'HttpRequest', 'upload', 'form', 'layer', 'element'], function () {
    ...
    $.ajax({
        type: 'get',
        url: Feng.ctxPath + '/spot/productivemine/fileDetail/' + $("#keyIdValue").val(),
        success: function (res) {
            ...
            for (let i = 0; i < uploadedFileArray.length; i++) {
                if (res.data[i].fileBusinessName == "规划图" || res.data[i].fileBusinessName == "图斑影像图") {
                    ...
                    tr.find('.file-delete').on('click', function (data) {
                        ...
                        var operation = function () {
                            ...
                            var ajax = new HttpRequest(Feng.ctxPath + '/spot/productivemine/deleteFileByFileId', 'post', function (data) {
                                Feng.success("删除成功!");
                                tr.remove();
                            }, function (data) {
