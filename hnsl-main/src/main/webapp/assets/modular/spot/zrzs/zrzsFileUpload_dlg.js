layui.use(['admin', 'HttpRequest', 'upload', 'form', 'layer', 'element'], function () {
    ...
    //获取associateid
    var ajaxassociateid = new HttpRequest(Feng.ctxPath + '/spot/zrzs/detail?id=' + keyIdValue, 'get');
    ...
    $.ajax({
        type: 'get',
        url: Feng.ctxPath + '/spot/zrzs/fileDetail/' + $("#keyIdValue").val(),
            ...
            for (let i = 0; i < uploadedFileArray.length; i++) {
                if (res.data[i].fileBusinessName == "规划图" || res.data[i].fileBusinessName == "图斑影像图" || res.data[i].fileBusinessName == "核查") {
                    ...
                    trHtml += `    <td>
                                    ...
                                    <button class="layui-btn layui-btn-xs layui-btn-danger file-delete" id="file-delete">删除</button>
                               </td>
                           </tr>`;
                    let tr = $(trHtml);
                    ...
                    tr.find('.file-delete').on('click', function (data) {
                        ...
                        var operation = function () {
                            ...
                            var ajax = new HttpRequest(Feng.ctxPath + '/spot/zrzs/deleteFileByFileId', 'post', function (data) {
                                Feng.success("删除成功!");
                                tr.remove();
                            }, function (data) {
