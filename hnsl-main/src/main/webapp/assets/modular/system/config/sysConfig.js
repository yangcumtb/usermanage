layui.use(['table', 'admin', 'HttpRequest','func'], function () {
    ...
    SysConfig.openAddDlg = function () {
        window.location.href = Feng.ctxPath + '/system/config/add';
    };
    ...
    SysConfig.openEditDlg = function (data) {
        func.open({
            title: '参数修改',
            content: Feng.ctxPath + '/system/config/edit?id=' + data.id,
    ...
    SysConfig.onDeleteItem = function (data) {
        var operation = function () {
            var ajax = new HttpRequest(Feng.ctxPath + "/system/config/delete",'post', function (data) {
                Feng.success("删除成功!");
                table.reload(SysConfig.tableId);
            }, function (data) {
    ...
    var tableResult = table.render({
        ...
        url: Feng.ctxPath + '/system/config/list',
        page: true,
        height: "full-100",
        cellMinWidth: 100,
