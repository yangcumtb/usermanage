layui.use(['layer', 'form', 'admin', 'HttpRequest'], function () {
    ...
    //获取详情信息，填充表单
    var ajax = new HttpRequest(Feng.ctxPath + "/system/notice/detail?noticeId=" + Feng.getUrlParam("noticeId"), "get");
    var result = ajax.start();
    form.val("noticeForm", result.data);

    // 表单提交事件
    form.on('submit(btnSubmit)', function (data) {
        var ajax = new HttpRequest(Feng.ctxPath + "/system/notice/edit",'post', function (data) {
            Feng.success("修改成功！");
