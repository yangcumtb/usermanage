...
layui.define(['layer'], function (exports) {
    ...
    admin.activeNav = function (url) {
        ...
        if ($a.length === 0) {
            ...
            $(headerDOM + '>.layui-nav>.layui-nav-item>a[nav-bind="home"]').parent().addClass('layui-this');
            return;
        };
    ...
    /** 右侧弹出 */
    admin.popupLeft = function (param) {
        param.anim = -1;
        param.offset = 'l';
        param.move = false;
        if (param.area === undefined) param.area = '336px';
        ...
        if (param.closeBtn === undefined) param.closeBtn = false;
        if (param.shadeClose === undefined) param.shadeClose = true;
        if (param.shade === undefined) param.shade = 0;
        if (param.skin === undefined) param.skin = 'layui-anim layui-anim-lr ';
        return admin.open(param);
    };
    ...
    $(document).on('click', '*[ew-href]', function () {
        ...
        if (href.substr(0, Feng.ctxPath.length) == Feng.ctxPath) {
            href = href.substr(Feng.ctxPath.length, href.length);
        }
        if (!href || href === '#') return;
        ...
        if (win.layui && win.layui.index) win.layui.index.openTab({title: title || '', url: Feng.ctxPath + href, end: end});
        else location.href = Feng.ctxPath + href;
    });
