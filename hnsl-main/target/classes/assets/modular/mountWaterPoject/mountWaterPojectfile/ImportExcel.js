layui.use(['table', 'layarea', 'admin', 'form', 'HttpRequest', 'func', 'upload', 'element', 'formSelects', 'layer'], function () {
    var $ = layui.jquery
        , upload = layui.upload
    ...
    form.on('select(repeatAction)', function (data) {
        repeatAction = data.value;
    });
    form.on('select(xmsjAction)', function (data) {
        xmsjAction = data.value;
    });
    form.on('select(xmsjMonth)', function (data) {
        xmsjMonth = data.value;
    });
    ...
    var uploadListIns = upload.render({
        elem: '#testList',
        elemList: $('#demoList'),
        url: '/spotmanage/mountwaterCommon/ImportExcelList',
        accept: 'file',
        ...
        before: function (obj) { //obj参数包含的信息，跟 choose回调完全一致
            this.data = {
                ...
                xmsjAction: xmsjAction,
                xmsjMonth: xmsjMonth,
            };
            if (repeatAction == null || xmsjAction == null || xmsjMonth  == null ) {
                return false
            }
