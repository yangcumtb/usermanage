layui.use(['HttpRequest', 'treeTable', 'func'], function () {
    ...
    Dict.openAddDlg = function () {
        func.open({
            width: "1000rem",
            title: '添加字典',
            content: Feng.ctxPath + '/system/dict/add?dictTypeId=' + $("#dictTypeId").val(),
    ...
    Dict.openEditDlg = function (data) {
        func.open({
            width: "1000rem",
            title: '修改字典',
            content: Feng.ctxPath + '/system/dict/edit?dictId=' + data.dictId,
            tableId: Dict.tableId,
            endCallback: function () {
                Dict.initTable(Dict.tableId);
            }
    ...
    Dict.onDeleteItem = function (data) {
        var operation = function () {
            var ajax = new HttpRequest(Feng.ctxPath + "/system/dict/delete",'post', function (data) {
                Feng.success("删除成功!");
                Dict.search();
            }, function (data) {
    ...
    Dict.initTable = function (dictId, data) {
        return treeTable.render({
            ...
            reqData: function (data, callback) {
                var ajax = new HttpRequest(Feng.ctxPath + '/system/dict/list?dictTypeId=' + $("#dictTypeId").val(),'get', function (res) {
                    callback(res.data);
                }, function (res) {
    ...
    $('#btnBack').click(function () {
        window.location.href = Feng.ctxPath + "/system/dictType";
    });
