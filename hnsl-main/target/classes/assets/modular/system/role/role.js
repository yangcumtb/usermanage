layui.use(['layer', 'form', 'table', 'admin', 'HttpRequest', 'func'], function () {
    ...
    Role.initColumn = function () {
        return [[
            {type: 'checkbox'},
            {field: 'roleId',  sort: true, title: '角色编号'},
            {field: 'roleName', align: "center", sort: true, title: '角色名称'},
            {field: 'roleKey', align: "center", sort: true, title: '权限字符'},
            {field: 'roleSort', align: "center", sort: true, title: '显示顺序'},
    ...
    Role.openAddRole = function () {
        func.open({
            width: "1000rem",
            title: '添加角色',
            content: Feng.ctxPath + '/system/role/role_add',
    ...
    Role.onEditRole = function (data) {
        func.open({
            width: "1000rem",
            title: '修改角色',
            content: Feng.ctxPath + "/system/role/role_edit?roleId=" + data.roleId,
            tableId: Role.tableId
        });
    };
    ...
    Role.onDeleteRole = function (data) {
        var operation = function () {
            var ajax = new HttpRequest(Feng.ctxPath + "/system/role/remove",'post', function () {
                Feng.success("删除成功!");
                table.reload(Role.tableId);
            }, function (data) {
    ...
    Role.roleAssign = function (data) {
        layer.open({
            ...
            area: ['500px', '400px'], //宽高
            fix: false,
            content: Feng.ctxPath + '/system/role/role_assign/' + data.roleId,
            end: function () {
                table.reload(Role.tableId);
            }
        });
    ...
    var tableResult = table.render({
        elem: '#' + Role.tableId,
        url: Feng.ctxPath + '/system/role/list',
        page: true,
        height: "full-98",
        cellMinWidth: 100,
