...
layui.use(['form', 'HttpRequest'], function () {
    ...
    var form = layui.form;
    var ajax = new HttpRequest(Feng.ctxPath + "/prominemanger/recoveredspot/location?id=" + Feng.getUrlParam("id"), "get");
    var result = ajax.start();
    if (result.data.sddl == null){
        result.data.sddl = '/'
    }
    if (result.data.fkywr == null){
        result.data.fkywr = '/'
    }
    ...
    function onEachFeature(feature, layer) {
        var popupContent = '<table class="table">' +
            ...
            '</td>' +
            '<tr>' +
            '<th rowspan="1">矿权编号：</th>' +
            '<td>' + result.data.kqbh +
            '</td>' +
            '<tr>' +
            '<th rowspan="1">二调地类：</th>' +
            '<td>' + result.data.sddl +
            '</td>' +
            '<tr>' +
            '<th rowspan="1">复垦时间：</th>' +
            '<td>' + result.data.fksj +
            '</td>' +
            '<tr>' +
            '<th rowspan="1">损毁土地复垦义务人：</th>' +
            '<td>' + result.data.fkywr +
            '</td>' +
            '<tr>' +
            '</tr>' +
            '</table>' + ''
