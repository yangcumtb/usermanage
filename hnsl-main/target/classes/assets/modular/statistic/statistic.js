layui.use(['table', 'admin', 'HttpRequest', 'func'], function () {
        ...
        Statistic.exportWord = function () {
            ...
            if (checkRows.data.length === 0) {
                ...
                table.exportFile(tableResult.config.id, checkRows.data, 'doc');
            }
        }
        ...
        var tableResult = table.render({
            ...
            url: Feng.ctxPath + '/statistic/lsyl/list',
            page: true,
            height: "full-100",
            cellMinWidth: 100,
         ...
         * 导出年份的报告
         */
        Statistic.onStatisticword = function (data) {
            window.open(Feng.ctxPath + "/statistic/lsyl/analyseReport");
        }
        ...
        table.on('tool(' + Statistic.tableId + ')', function (obj) {
            ...
            if (layEvent === 'statisticword') {
                Statistic.onStatisticword(data);
            }
        });
    }
)
