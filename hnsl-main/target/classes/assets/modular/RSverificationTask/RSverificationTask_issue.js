layui.use(['table', 'layarea', 'admin', 'form', 'HttpRequest', 'func'], function () {
    ...
    form.on('select(tbtype)', function (data) {
        if (data.value == 1) {
            $("#RSverificationTaskTable").show();
        } else {
            $("#RSverificationTaskTable").hide();
        }
    })
    ...
    RSverificationTask.search = function () {
        ...
        // RSverificationTask.queryData.scdw = $("#scdw").val();
        // RSverificationTask.queryData.jsdw = $("#jsdw").val();
        // RSverificationTask.queryData.xmmc = $("#xmmc").val();
    ...
    RSverificationTask.jumpEditPage = function (data) {
        window.location.href = Feng.ctxPath + '/RSverification/RSverificationTask/edit?id=' + data.id
    };
     ...
     * @param data
     */
    RSverificationTask.onCheck = function (data) {
        func.open({
            width: "1000rem",
            title: '审核',
            content: Feng.ctxPath + '/verification/verificationTask/checkHtml?id=' + data.id,
            tableId: RSverificationTask.tableId,
        });
    }
    // 渲染表格
    var tableResult = table.render({
        ...
        url: Feng.ctxPath + '/RSverification/RSverificationTask/issueList',
        page: true,
        height: "full-100",
        cellMinWidth: 120,
    ...
    table.on('tool(' + RSverificationTask.tableId + ')', function (obj) {
        ...
        if (layEvent === 'edit') {
            ...
            RSverificationTask.onCheck(data);
        }
    });
});
