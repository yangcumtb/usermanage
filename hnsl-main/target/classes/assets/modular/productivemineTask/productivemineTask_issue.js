layui.use(['table', 'layarea', 'admin', 'form', 'HttpRequest', 'func'], function () {
    ...
    VerificationTask.jumpAddPage = function () {
        window.location.href = Feng.ctxPath + '/productivemineTask/add'
    };
    ...
    VerificationTask.jumpEditPage = function (data) {
        window.location.href = Feng.ctxPath + '/productivemineTask/edit?id=' + data.id
    };
    ...
    VerificationTask.onDeleteItem = function (data) {
        var operation = function () {
            var ajax = new HttpRequest(Feng.ctxPath + "/productivemineTask/delete", 'post', function (data) {
                Feng.success("删除成功!");
                table.reload(VerificationTask.tableId);
            }, function (data) {
    ...
    VerificationTask.jumpEditPage = function (data) {
        func.open({
            width: "1000rem",
            title: '查看详细',
            content: Feng.ctxPath + '/productivemineTask/detailHtml?id=' + data.id,
    ...
    VerificationTask.onDistribute = function (data) {
        ...
        func.open({
            width: "1000rem",
            title: '外业核查任务下发',
            content: Feng.ctxPath + '/productivemineTask/distributeHtml?id=' + data.id,
            tableId: VerificationTask.tableId,
        })
    }
    /**
    ...
    VerificationTask.onCheck = function (data) {
        func.open({
            width: "1000rem",
            title: '审核',
            content: Feng.ctxPath + '/productivemineTask/checkHtml?id=' + data.id,
            tableId: VerificationTask.tableId,
        });
    }
    // 渲染表格
    var tableResult = table.render({
        ...
        url: Feng.ctxPath + '/productivemineTask/issueList',
        page: true,
        height: "full-100",
        cellMinWidth: 120,
