 ...
 */
package cn.hnsl.core.treebuild;

import cn.hnsl.core.treebuild.abst.AbstractTreeBuildFactory;
import cn.hnsl.model.tree.Tree;
import lombok.Data;
...
public class DefaultTreeBuildFactory<T extends Tree> extends AbstractTreeBuildFactory<T> {
    ...
    protected List<T> afterBuild(List<T> nodes) {
        ...
        for (T node : nodes) {
            if (node.getNodeParentId().equals(rootParentId.trim())) {
                results.add(node);
            }
        }
        return results;
