...
layui.use(['form', 'admin', 'HttpRequest', 'upload'], function () {
    var $ = layui.jquery;
    ...
    //获取详情信息，填充表单
    var ajax = new HttpRequest(Feng.ctxPath + "/excelExportDeploy/detail?id=" + Feng.getUrlParam("id"), "get");
    var result = ajax.start();
    form.val('excelExportDeployForm', result.data);
	$("#src").text(result.data.template);
    //表单提交事件
    form.on('submit(btnSubmit)', function (data) {
        var ajax = new HttpRequest(Feng.ctxPath + "/excelExportDeploy/editItem","post", function (data) {
            Feng.success("更新成功！");
